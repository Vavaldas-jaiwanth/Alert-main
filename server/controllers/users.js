...
module.exports = {
  signup: async (req, res) => {
    const { firstName, lastName, code, email, password, phoneNumber, role } =
      req.body; // Include 'role' here
    console.log({
      firstName,
      lastName,
      code,
    ...
    try {
      if (!isValid) {
        ...
        if (exist) {
          ...
          return res
            .status(404)
            .json({ message: "Email is already in use", errors });
        } else {
  ...
  signin: async (req, res) => {
    ...
    try {
      ...
      if (!user) {
        errors.email =
          "Email does not exist! Please enter the correct Email or create an account";
        return res.status(404).json(errors);
      }
      ...
      if (!passwordMatch) {
        ...
        return res
          .status(404)
          .json({ message: "Password not matched", errors });
      }
